...
if ( close_session == "1" )
{
document.location.replace('DHCPTable.asp');
}
else
{
document.location.replace('DHCPTable.asp?session_id=<% nvram_get("session_key"); %>');
}
}
function init()
{
...
document.write('<td>' + table[i].time + '</td>');
if(ret == 1)
document.write('<td>&nbsp;</td>');
else
{
document.write('<td>');
draw_button("javascript:to_delete('" + ip[3] + "')", sbutton.del);
document.write('</td>');
}
document.write('</tr>');
if(ret == 1)

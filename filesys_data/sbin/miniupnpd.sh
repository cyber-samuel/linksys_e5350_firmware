...
$IPTABLES -t nat -F upnp_nat 1>/dev/null 2>&1
#rmeoving the rule to MINIUPNPD
$IPTABLES -t nat -D PREROUTING -i $WAN_IF -j upnp_nat 1>/dev/null 2>&1
$IPTABLES -t nat -X upnp_nat 1>/dev/null 2>&1

#removing the MINIUPNPD chain for filter
$IPTABLES -t filter -F upnp_forward 1>/dev/null 2>&1
#adding the rule to MINIUPNPD
$IPTABLES -t filter -D FORWARD -i $WAN_IF ! -o $WAN_IF -j upnp_forward 1>/dev/null 2>&1
$IPTABLES -t filter -X upnp_forward 1>/dev/null 2>&1

if [ "$1" == "init" ]; then
	MINIUPNPD_FILE=/etc/miniupnpd.conf
	if [ "$OP_MODE" = "0" ]; then
		LAN_IPADDR=`nvram_get 2860 switch_ipaddr`
	else
	...
	$IPTABLES -t nat -N upnp_nat
	$IPTABLES -t nat -A PREROUTING -i $WAN_IF -j upnp_nat
	$IPTABLES -t filter -N upnp_forward
	$IPTABLES -t filter -A FORWARD -i $WAN_IF ! -o $WAN_IF -j upnp_forward

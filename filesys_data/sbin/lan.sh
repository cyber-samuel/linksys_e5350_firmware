#!/bin/sh
#
# $Id: //WIFI_SOC/MP/SDK_5_0_0_0/RT288x_SDK/source/user/rt2880_app/scripts/lan.sh#1 $
#
# usage: wan.sh
#
...
if [ "$host" = "" ]; then
	host="BLK"
	nvram_set 2860 HostName BLK
fi
hostname $host
echo "127.0.0.1 localhost.localdomain localhost" > /etc/hosts
echo "$ip $host.belkin.com $host" >> /etc/hosts
echo "$ip myrouter.local myroyter.local" >> /etc/hosts
...
#first stop lan
if [ "$CONFIG_USER_ISC_DHCP" = "y" ]; then
	lan stop 
fi

#Not start dhcpd without gateway mode
if [ "$opmode" = "1" -a "$dhcp" = "1" ]; then
	if [ "$CONFIG_USER_ISC_DHCP" = "y" ]; then
		echo "Start dhcpd ..."
		lan start 
	else
...
echo 66560 > /proc/sys/net/core/rmem_max
# increase the oom_kill threshold
